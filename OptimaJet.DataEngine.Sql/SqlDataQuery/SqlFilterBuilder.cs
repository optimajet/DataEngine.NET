...
public sealed class SqlFilterBuilder : FilterVisitor, ISqlFilterBuilder
{
    ...
    public Query Build(IFilter filter)
    {
    ...
    /// Current query
    /// </summary>
    private Query Query => Queries.Peek();

    private Stack<Query> Queries { get; }
    ...
    private void AddCondition(Func<Query, Query> fAddQueryComponent)
    {
        if (UseOr)
        {
            Query.Or();
